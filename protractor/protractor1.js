...
it('Should properly update attendance',function(){
	element.all(by.css('[data-ng-bind="member.displayName"]')).then(function(arr){
		expect(arr.length).toBe(2);
		arr[1].click();
		expect(element(by.css('[data-ng-bind="meeting.membersPresent"]')).getText()).toBe('1');
	});
	element(by.buttonText('Save Attendance')).click();
	expect(element(by.css('[data-ng-bind="saveMessage"]')).getText()).toContain('Attendance last saved at');
});
...
it('Should test the calendar to see if the meeting is showing',function(){
	ptor.executeScript('window.scrollTo(0,0);').then(function () {
	    element(by.css('[ng-if="committeeTemplates.current"]')).click();
	});
